...
`serve` 的 host 与 port 独立解析：命令行显式参数优先于配置文件中的 `server.host` / `server.port`，未提供时使用配置，二者都缺省时分别使用 `0.0.0.0` 与 `5090`。配置热加载改动 host 或 port 不会切换已经绑定的监听地址，只打印告警并提示重启。`run.pid` 与项目注册表记录实际监听端口。启动时对实际监听地址进行安全判断，实际监听地址为 `0.0.0.0` 且 `adminEnabled=true` 时打印 WARNING。

> 2026-09 修复：旧实现只看命令行（默认 `0.0.0.0:5090`），配置里的 `server.host/port` 完全不参与监听，
> README "配置里写 `host: "127.0.0.1"` 只绑本机"的安全建议因此是无效的；暴露告警与 doctor 又按配置值判断，三处口径不一。
