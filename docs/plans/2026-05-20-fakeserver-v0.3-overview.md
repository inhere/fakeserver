...
|---|---|---|---|---|---|---|
| **1** | `internal/registry` 包（projects.json 读写 + 跨进程文件锁 + PID 文件）+ serve 启动期 Upsert | `internal/registry/{store,lock,pid}.go` + `internal/cli/serve.go` 接入 | 待 Task 1 spike 决定（gofrs/flock vs 自写 syscall）| v0.2 | ~600 行 | 待开始 |
