...
| 1R2 | 1R 验收返工 | 已完成 | `ed7ba87` / `cd3cf2a` |
| 2 | fallback 可配置 + admin 默认仅本机（2.1-2.2） | 已完成（验收返工见 2R） | `b87fe03` / `d167ca1` |
| 2R | 阶段 2 验收返工（第 1-6 条） | 已完成 | `37964d1` / `4787228` / `0a59ccf` / `c3010c3` |
| 3 | 模板两项（3.1-3.2） | 已完成 | `2701a21` / `f3f0934` / `7f963c3` |
...
2R 验收（2026-09-12）：通过。实测 echo 标识头、`--host 0.0.0.0` 覆盖时横幅标注 `(loopback only)` 且不告警、绑定具体非回环 IP 时标注、
`0.0.0.0 + adminAllowRemote` 才告警、远程 403 英文文案均符合；容器内全量测试与 `-race` 通过。遗留两项**并入阶段 3 一起做**：

- 第 6 条要求的 `isNonLoopbackListenHost` 表驱动单测没有加（只改了既有测试）。补上，覆盖 `127.0.0.1`、`::1`、`localhost`、`0.0.0.0`、`::`、`192.168.1.10`、
  一个非 localhost 的主机名，并据此确认 `localhost` 被当作回环；同时覆盖"是否告警"的判定（非回环 × adminEnabled × adminAllowRemote）。
- 进度表 2R 行的"待提交"改为实际提交号 `37964d1` / `4787228` / `0a59ccf` / `c3010c3`。
